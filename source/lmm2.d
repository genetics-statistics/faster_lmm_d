...
class LMM2{
  ...
  void LL_brent(self,h,X=None,REML=False){
  ...
  void LL(self,h,X=None,stack=True,REML=False){
      //"""
      ...
      if(X is None){
        ...
        self.X0t_stack[sval,(self.q)] = matrixMult(self.Kve.T,X)[sval,0];
        X = self.X0t_stack;
      }
      n = float(self.N);
      q = float(X.shape[1]);
  ...
  void getMax(self,H, X=None,REML=False){
    ...
    foreach(i; range(1,n-2)){
      if(self.LLs[i-1] < self.LLs[i] && self.LLs[i] > self.LLs[i+1]){
  ...
  void fit(self,X=None,ngrids=100,REML=True){
  ...
  void association(self,X,h=None,stack=True,REML=True,returnBeta=False){
    //"""
    ...
    if(false){
      ...
      writeln("h=",h);
      writeln("q=",self.q);
      writeln("self.Kve=",self.Kve);
      writeln("X0t_stack=",self.X0t_stack.shape,self.X0t_stack);
    }
  ...
  void tstat(self,beta,var,sigma,q,log=false){
  ...
  void plotFit(self,color="b-",title=""){
