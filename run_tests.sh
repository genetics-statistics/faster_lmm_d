...
./build/faster_lmm_d --control=data/genenetwork/BXD.json --pheno=data/small.pheno --geno=data/small.geno --test-kinship=true --cmd=run $*
if [ $? -ne 0 ]; then echo "ERR1a small test failed" ; exit 1 ; fi

echo "Running small_na test"
./build/faster_lmm_d --control=data/genenetwork/BXD.json --pheno=data/small_na.pheno --geno=data/small_na.geno --test-kinship=true --cmd=run $*
...
time ./build/faster_lmm_d --control=data/genenetwork/BXD.json --pheno=data/test8000.pheno --geno=data/test8000.geno --test-kinship=true --cmd=run $*
